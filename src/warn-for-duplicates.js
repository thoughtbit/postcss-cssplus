...
const msg = (name) => (
  `Warning: postcss-cssplus found a duplicate plugin ('${ name }') ` +
  "in your postcss plugins. " +
  `This might be inefficient. You should remove '${ name }' from your ` +
  "postcss plugin list since it's already included by postcss-cssplus."
)

let shouldGlobalWarn = true
const globalWarning = (
  "Note: If, for a really specific reason, postcss-cssplus warnings are " +
  ...
  "postcss-cssplus to 'false'."
)
export const spotted = []

const warnForDuplicates = postcss.plugin(
  "postcss-cssplus-warn-for-duplicates",
